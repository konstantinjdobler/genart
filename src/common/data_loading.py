...
# loading ArtEmis without (with fixed) annoations
class ArtEmisDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        self.train_set = ArtEmisAnnotatedImageDataset(str(self.image_subfolder), str(
            self.annotation_path), transform=self.transform, fast_debug=self.fast_debug)
