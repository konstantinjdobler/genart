# Taken from https://github.com/robbiebarrat/art-DCGAN/blob/master/genre-scraper.py
...
parser.add_argument("--genre", help="which genre to scrape",
                    choices=genre_list, default=None)
parser.add_argument("--style", help="which style to scrape",
                    choices=style_list, default=None)
parser.add_argument("--num_pages", type=int,
                    help="number of pages to scrape (leave blank to download all of them)", default=1000)
...
def get_painting_list(count, typep, searchword):
    try:
        ...
        url = "https://www.wikiart.org/en/paintings-by-%s/%s/%d" % (
            typep, searchword, count)
...
def main(typep, searchword, num_pages, output_dir):
    ...
    wikiart_pages = threadpool.starmap(get_painting_list, zip(
        numbers, itertools.repeat(typep), itertools.repeat(searchword)))
    threadpool.close()
    threadpool.join()
    ...
    threadpool = Pool(multiprocessing.cpu_count()-1)
    threadpool.starmap(downloader, zip(enumerate(items), itertools.repeat(
        searchword), itertools.repeat(output_dir)))
    threadpool.close
    threadpool.close()
...
if __name__ == '__main__':
    ...
    searchword, typep = (args.genre, 'genre') if args.genre is not None else (
        args.style, 'style')
