...
# TODO: add support to add the features of the dataset
...
def _get_csv_entries(csv_file='first_artemis_paintings.txt'):

    pd_csv = pd.read_csv(csv_file, delimiter="\n",
                         sep=",", header=None, names=["url"])
    print(pd_csv["url"])
    return pd_csv["url"]
...
if __name__ == '__main__':
    ...
    with open("artemis_labels.json", "w") as outfile:
        json_file = json.dump(json_file, outfile, indent=2)

    print(f'number of successfully scraped painting links: {len(json_list)}	')

    with open("not_found_list.txt", "a+") as outfile:
